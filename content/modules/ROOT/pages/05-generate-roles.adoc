...
You have a working playbook that sets up web servers with Apache and a status page, configures MariaDB on the database tier, creates users, and deploys templates. In this module, you will use the coding assistant to convert that playbook into an *Ansible role* — the standard way to organize and reuse automation in production.
...
. If there is a `templates/` directory, confirm it contains `motd.j2` and `index.html.j2`.
=====
======
