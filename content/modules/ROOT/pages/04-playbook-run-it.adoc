...
. Click the *node1 Web* tab at the top of the screen. You should see a styled status page showing node1's hostname, IP address, OS version, and architecture — all rendered from Ansible facts by the Jinja2 template you deployed.

. Click the *node2 Web* tab. Notice the hostname and IP address changed to node2's values, even though both servers used the same template file. This is the power of templates — one source file, customized output per host.

. Switch back to the *Control* tab and verify from the command line as well:
+
[source,role="execute"]
----
