...
The playbook deployed an HTML status page to the web servers using the `index.html.j2` template. Each web server now has a page showing its own system information.

. View the status page on node1:
+
[source,role="execute"]
----
curl node1
----
+
You should see an HTML page containing node1's hostname, IP address, OS version, and architecture. This is the Jinja2 template rendered with node1's specific Ansible facts.

. View the status page on node2:
+
[source,role="execute"]
----
curl node2
----
+
Notice the hostname and IP address changed to node2's values, even though both servers used the same template file. This is the power of templates — one source file, customized output per host.
